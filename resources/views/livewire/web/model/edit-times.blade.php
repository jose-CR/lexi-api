    ...
    @foreach($times as $type => $time)
        <div class="mb-6 p-4 border rounded-lg shadow-sm bg-gray-50 dark:bg-gray-800">
            <div class="flex justify-between items-center mb-3">
                <h3 class="font-semibold text-gray-800 dark:text-gray-100">{{ ucfirst($type) }}</h3>
                <button type="button" wire:click="removeTime('{{ $type }}')" class="px-3 py-1 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700">
                    Eliminar
                </button>
            </div>
